...
jq(document).ready( function() {
	...
	jq('body').on( 'click', 'div.activity', function(event) {
		...
		if ( target.hasClass('acomment-reply') || target.parents('.acomment-reply').length ) {
			...
			if ( target.hasClass('acomment-reply') ) {
				...
				target = target.closest('.acomment-reply');
			}
			...
			/**
			 * Since there is a bug with the iOS keyboard and input
			 * fields using .focus(), just do this for Android
			 */
	...
	jq( document ).on( 'heartbeat-tick', function( e, data ) {
		...
		jq( '#buddypress ul.activity-list' ).prepend( '<li class="load-newest"><a href="#newest" class="no-transition">' + BP_DTheme.newest + '</a></li>' );
	});
});
