...
class BP_Legacy extends BP_Theme_Compat {
	...
	protected function setup_globals() {
		...
		if( class_exists( 'AppPresser_3_Theme_Setup' ) ) {
			$ap3_setup = new AppPresser_3_Theme_Setup();
			$version = $ap3_setup::VERSION;
		} else {
		...
		$bp            = buddypress();
		$this->id      = 'legacy';
		$this->name    = __( 'BuddyPress Legacy', 'ap3-ion-theme' );
	...
	public function enqueue_scripts() {
		...
		$params = apply_filters( 'bp_core_get_js_strings', array(
			'accepted'            => __( 'Accepted', 'ap3-ion-theme' ),
			'close'               => __( 'Close', 'ap3-ion-theme' ),
			'comments'            => __( 'comments', 'ap3-ion-theme' ),
			'leave_group_confirm' => __( 'Are you sure you want to leave this group?', 'ap3-ion-theme' ),
			'mark_as_fav'	      => __( 'Favorite', 'ap3-ion-theme' ),
			'my_favs'             => __( 'My Favorites', 'ap3-ion-theme' ),
			'rejected'            => __( 'Rejected', 'ap3-ion-theme' ),
			'remove_fav'	      => __( 'Remove Favorite', 'ap3-ion-theme' ),
			'show_all'            => __( 'Show all', 'ap3-ion-theme' ),
			'show_all_comments'   => __( 'Show all comments for this thread', 'ap3-ion-theme' ),
			'show_x_comments'     => __( 'Show all %d comments', 'ap3-ion-theme' ),
			'unsaved_changes'     => __( 'Your profile has unsaved changes. If you leave the page, the changes will be lost.', 'ap3-ion-theme' ),
			'view'                => __( 'View', 'ap3-ion-theme' ),
		) );
		...
		if ( bp_is_active( 'messages', 'star' ) && bp_is_user_messages() ) {
			wp_localize_script( $asset['handle'], 'BP_PM_Star', array(
				'strings' => array(
					'text_unstar'  => __( 'Unstar', 'ap3-ion-theme' ),
					'text_star'    => __( 'Star', 'ap3-ion-theme' ),
					'title_unstar' => __( 'Starred', 'ap3-ion-theme' ),
					'title_star'   => __( 'Not starred', 'ap3-ion-theme' ),
					'title_unstar_thread' => __( 'Remove all starred messages in this thread', 'ap3-ion-theme' ),
					'title_star_thread'   => __( 'Star the first message in this thread', 'ap3-ion-theme' ),
				),
...
function bp_legacy_theme_post_update() {
	...
	if ( empty( $_POST['content'] ) )
		exit( '-1<div id="message" class="error bp-ajax-message"><p>' . __( 'Please enter some content to post.', 'ap3-ion-theme' ) . '</p></div>' );
	...
	if ( empty( $activity_id ) )
		exit( '-1<div id="message" class="error bp-ajax-message"><p>' . __( 'There was a problem posting your update. Please try again.', 'ap3-ion-theme' ) . '</p></div>' );
...
function bp_legacy_theme_new_activity_comment() {
	...
	$feedback = __( 'There was an error posting your reply. Please try again.', 'ap3-ion-theme' );

	if ( empty( $_POST['content'] ) ) {
		exit( '-1<div id="message" class="error bp-ajax-message"><p>' . esc_html__( 'Please do not leave the comment area blank.', 'ap3-ion-theme' ) . '</p></div>' );
	}
...
function bp_legacy_theme_delete_activity() {
	...
	if ( ! bp_activity_delete( array( 'id' => $activity->id, 'user_id' => $activity->user_id ) ) )
		exit( '-1<div id="message" class="error bp-ajax-message"><p>' . __( 'There was a problem when deleting. Please try again.', 'ap3-ion-theme' ) . '</p></div>' );
...
function bp_legacy_theme_mark_activity_favorite() {
	...
	if ( bp_activity_add_user_favorite( $_POST['id'] ) )
		_e( 'Remove Favorite', 'ap3-ion-theme' );
	else
		_e( 'Favorite', 'ap3-ion-theme' );
...
function bp_legacy_theme_ajax_invite_user() {
	...
	if ( 'invite' == $_POST['friend_action'] ) {
		...
		echo '<div class="action">
				<a class="button remove" href="' . wp_nonce_url( $uninvite_url, 'groups_invite_uninvite_user' ) . '" id="uid-' . esc_attr( $user->id ) . '">' . __( 'Remove Invite', 'ap3-ion-theme' ) . '</a>
			  </div>';

		if ( 'is_pending' == $user_status ) {
			echo '<p class="description">' . sprintf( __( '%s has previously requested to join this group. Sending an invitation will automatically add the member to the group.', 'ap3-ion-theme' ), $user->user_link ) . '</p>';
		}
...
function bp_legacy_theme_ajax_addremove_friend() {
	...
	if ( 'is_friend' == BP_Friends_Friendship::check_is_friend( bp_loggedin_user_id(), $friend_id ) ) {
		...
		if ( ! friends_remove_friend( bp_loggedin_user_id(), $friend_id ) ) {
			echo __( 'Friendship could not be canceled.', 'ap3-ion-theme' );
		} else {
			echo '<a id="friend-' . esc_attr( $friend_id ) . '" class="add" rel="add" title="' . __( 'Add Friend', 'ap3-ion-theme' ) . '" href="' . wp_nonce_url( bp_loggedin_user_domain() . bp_get_friends_slug() . '/add-friend/' . $friend_id, 'friends_add_friend' ) . '">' . __( 'Add Friend', 'ap3-ion-theme' ) . '</a>';
		}
	...
	// Trying to request friendship
		...
		if ( ! friends_add_friend( bp_loggedin_user_id(), $friend_id ) ) {
			echo __(' Friendship could not be requested.', 'ap3-ion-theme' );
		} else {
			echo '<a id="friend-' . esc_attr( $friend_id ) . '" class="remove" rel="remove" title="' . __( 'Cancel Friendship Request', 'ap3-ion-theme' ) . '" href="' . wp_nonce_url( bp_loggedin_user_domain() . bp_get_friends_slug() . '/requests/cancel/' . $friend_id . '/', 'friends_withdraw_friendship' ) . '" class="requested">' . __( 'Cancel Friendship Request', 'ap3-ion-theme' ) . '</a>';
		}
	...
	// Trying to cancel pending request
		...
		if ( friends_withdraw_friendship( bp_loggedin_user_id(), $friend_id ) ) {
			...
			echo __("Friendship request could not be cancelled.", 'ap3-ion-theme');
		}

	// Request already pending
	} else {
		echo __( 'Request Pending', 'ap3-ion-theme' );
	}
...
function bp_legacy_theme_ajax_accept_friendship() {
	...
	if ( ! friends_accept_friendship( (int) $_POST['id'] ) )
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem accepting that request. Please try again.', 'ap3-ion-theme' ) . '</p></div>';
...
function bp_legacy_theme_ajax_reject_friendship() {
	...
	if ( ! friends_reject_friendship( (int) $_POST['id'] ) )
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem rejecting that request. Please try again.', 'ap3-ion-theme' ) . '</p></div>';
	...
	if ( ! groups_is_user_member( bp_loggedin_user_id(), $group->id ) ) {
		if ( 'public' == $group->status ) {
			...
			if ( ! groups_join_group( $group->id ) ) {
				_e( 'Error joining group', 'ap3-ion-theme' );
			} else {
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="leave-group" rel="leave" title="' . __( 'Leave Group', 'ap3-ion-theme' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'leave-group', 'groups_leave_group' ) . '">' . __( 'Leave Group', 'ap3-ion-theme' ) . '</a>';
			}
			...
			if ( groups_check_user_has_invite( bp_loggedin_user_id(), $group->id ) ) {
				...
				if ( ! groups_accept_invite( bp_loggedin_user_id(), $group->id ) ) {
					_e( 'Error requesting membership', 'ap3-ion-theme' );
				} else {
					echo '<a id="group-' . esc_attr( $group->id ) . '" class="leave-group" rel="leave" title="' . __( 'Leave Group', 'ap3-ion-theme' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'leave-group', 'groups_leave_group' ) . '">' . __( 'Leave Group', 'ap3-ion-theme' ) . '</a>';
				}
			...
			// Otherwise, it's a Request Membership button
				...
				if ( ! groups_send_membership_request( bp_loggedin_user_id(), $group->id ) ) {
					...
					echo '<a id="group-' . esc_attr( $group->id ) . '" class="membership-requested" rel="membership-requested" title="' . __( 'Membership Requested', 'ap3-ion-theme' ) . '" href="' . bp_get_group_permalink( $group ) . '">' . __( 'Membership Requested', 'ap3-ion-theme' ) . '</a>';
				}
			}
		}
		...
		if ( ! groups_leave_group( $group->id ) ) {
			_e( 'Error leaving group', 'ap3-ion-theme' );
		} elseif ( 'public' == $group->status ) {
			echo '<a id="group-' . esc_attr( $group->id ) . '" class="join-group" rel="join" title="' . __( 'Join Group', 'ap3-ion-theme' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'join', 'groups_join_group' ) . '">' . __( 'Join Group', 'ap3-ion-theme' ) . '</a>';
		} elseif ( 'private' == $group->status ) {
			echo '<a id="group-' . esc_attr( $group->id ) . '" class="request-membership" rel="join" title="' . __( 'Request Membership', 'ap3-ion-theme' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'request-membership', 'groups_send_membership_request' ) . '">' . __( 'Request Membership', 'ap3-ion-theme' ) . '</a>';
		}
	}
...
function bp_legacy_theme_ajax_close_notice() {
	...
	if ( ! isset( $_POST['notice_id'] ) ) {
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem closing the notice.', 'ap3-ion-theme' ) . '</p></div>';
...
function bp_legacy_theme_ajax_messages_send_reply() {
	...
	?>
		...
		<div class="message-box new-message <?php bp_the_thread_message_css_class(); ?>">
			<div class="message-metadata">
				...
				<strong><a href="<?php echo bp_loggedin_user_domain(); ?>"><?php bp_loggedin_user_fullname(); ?></a> <span class="activity"><?php printf( __( 'Sent %s', 'ap3-ion-theme' ), bp_core_time_since( bp_core_current_time() ) ); ?></span></strong>
	...
	<?php
		...
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem sending that reply. Please try again.', 'ap3-ion-theme' ) . '</p></div>';
	}
...
function bp_legacy_theme_ajax_message_markunread() {
	...
	if ( ! isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem marking messages as unread.', 'ap3-ion-theme' ) . '</p></div>';
...
function bp_legacy_theme_ajax_message_markread() {
	...
	if ( ! isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __('There was a problem marking messages as read.', 'ap3-ion-theme' ) . '</p></div>';
...
function bp_legacy_theme_ajax_messages_delete() {
	...
	if ( ! isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem deleting messages.', 'ap3-ion-theme' ) . '</p></div>';

	} else {
		$thread_ids = wp_parse_id_list( $_POST['thread_ids'] );
		messages_delete_thread( $thread_ids );

		_e( 'Messages deleted.', 'ap3-ion-theme' );
	}
