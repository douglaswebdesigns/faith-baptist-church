...
<form action="<?php bp_group_admin_form_action(); ?>" name="group-settings-form" id="group-settings-form" class="standard-form list" method="post" enctype="multipart/form-data" role="main">
...
<?php if ( bp_is_group_admin_screen( 'edit-details' ) ) : ?>
	...
	<p class="padding">
		<label for="group-notifiy-members">
			<input type="checkbox" name="group-notify-members" value="1" /> <?php _e( 'Notify group members of these changes via email', 'ap3-ion-theme' ); ?>
		</label>
	</p>

	<?php do_action( 'bp_after_group_details_admin' ); ?>

	<p class="padding"><input type="submit" class="button button-primary" value="<?php esc_attr_e( 'Save Changes', 'ap3-ion-theme' ); ?>" id="save" name="save" /></p>
...
<?php if ( bp_is_group_admin_screen( 'group-settings' ) ) : ?>
	...
	<?php if ( bp_is_active( 'forums' ) ) : ?>
		...
		<?php if ( bp_forums_is_installed_correctly() ) : ?>
			...
			<div class="checkbox">
				<label class="item item-input"><input type="checkbox" name="group-show-forum" id="group-show-forum" value="1"<?php bp_group_show_forum_setting(); ?> /> <?php _e( 'Enable discussion forum', 'ap3-ion-theme' ); ?></label>
			</div>
	...
	<h4 class="padding"><?php _e( 'Privacy Options', 'ap3-ion-theme' ); ?></h4>
	...
	<h4 class="padding"><?php _e( 'Group Invitations', 'ap3-ion-theme' ); ?></h4>
...
<?php if ( bp_is_group_admin_screen( 'group-avatar' ) ) : ?>
	...
	<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
			...
			<?php if ( bp_get_group_has_avatar() ) : ?>
				...
				<p><?php _e( "If you'd like to remove the existing avatar but not upload a new one, please use the delete avatar button.", 'ap3-ion-theme' ); ?></p>

				<?php bp_button( array( 'id' => 'delete_group_avatar', 'component' => 'groups', 'wrapper_id' => 'delete-group-avatar-button', 'link_class' => 'edit', 'link_href' => bp_get_group_avatar_delete_link(), 'link_title' => __( 'Delete Avatar', 'ap3-ion-theme' ), 'link_text' => __( 'Delete Avatar', 'ap3-ion-theme' ) ) ); ?>
	...
	<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
		...
		<h4><?php _e( 'Crop Avatar', 'ap3-ion-theme' ); ?></h4>
		...
		<input type="submit" name="avatar-crop-submit" class="button button-primary" id="avatar-crop-submit" value="<?php esc_attr_e( 'Crop Image', 'ap3-ion-theme' ); ?>" />
...
<?php if ( bp_is_group_admin_screen( 'manage-members' ) ) : ?>
	...
	<div class="bp-widget">
		<h4 class="padding"><?php _e( 'Administrators', 'ap3-ion-theme' ); ?></h4>

		<?php if ( bp_has_members( '&include='. bp_group_admin_ids() ) ) : ?>

		<ul id="admins-list" class="item-list single-line list">

			<?php while ( bp_members() ) : bp_the_member(); ?>
			<li class="item">
				<?php echo bp_core_fetch_avatar( array( 'item_id' => bp_get_member_user_id(), 'type' => 'thumb', 'width' => 30, 'height' => 30, 'alt' => sprintf( __( 'Profile picture of %s', 'ap3-ion-theme' ), bp_get_member_name() ) ) ); ?>
				<h5>
					<a href="<?php bp_member_permalink(); ?>"> <?php bp_member_name(); ?></a>
					<?php if ( count( bp_group_admin_ids( false, 'array' ) ) > 1 ) : ?>
					<span class="small">
						<a class="button confirm admin-demote-to-member" href="<?php bp_group_member_demote_link( bp_get_member_user_id() ); ?>"><?php _e( 'Demote to Member', 'ap3-ion-theme' ); ?></a>
					</span>
					<?php endif; ?>
				</h5>
			</li>
	...
	<?php if ( bp_group_has_moderators() ) : ?>
		<div class="bp-widget">
			<h4 class="padding"><?php _e( 'Moderators', 'ap3-ion-theme' ); ?></h4>

			<?php if ( bp_has_members( '&include=' . bp_group_mod_ids() ) ) : ?>
				<ul id="mods-list" class="item-list single-line list">
					...
					<li class="item">
						...
						<h5 class="item-title">
							<a href="<?php bp_member_permalink(); ?>"> <?php bp_member_name(); ?></a>
							<span class="small">
								<a href="<?php bp_group_member_promote_admin_link( array( 'user_id' => bp_get_member_user_id() ) ); ?>" class="button confirm mod-promote-to-admin" title="<?php esc_attr_e( 'Promote to Admin', 'ap3-ion-theme' ); ?>"><?php _e( 'Promote to Admin', 'ap3-ion-theme' ); ?></a>
								<a class="button confirm mod-demote-to-member" href="<?php bp_group_member_demote_link( bp_get_member_user_id() ); ?>"><?php _e( 'Demote to Member', 'ap3-ion-theme' ); ?></a>
							</span>
						</h5>
					</li>
					<?php endwhile; ?>
	...
	<div class="bp-widget">
		<h4 class="padding"><?php _e("Members", 'ap3-ion-theme'); ?></h4>
		...
		<?php if ( bp_group_has_members( 'per_page=15&exclude_banned=false' ) ) : ?>
			...
			<ul id="members-list" class="item-list single-line list">
				<?php while ( bp_group_members() ) : bp_group_the_member(); ?>

					<li class="item <?php bp_group_member_css_class(); ?>">
						<?php bp_group_member_avatar_mini(); ?>

						<h5 class="item-title">
							<?php bp_group_member_link(); ?>

							<?php if ( bp_get_group_member_is_banned() ) _e( '(banned)', 'ap3-ion-theme' ); ?>

							<span class="small">

							<?php if ( bp_get_group_member_is_banned() ) : ?>

								<a href="<?php bp_group_member_unban_link(); ?>" class="button confirm member-unban" title="<?php esc_attr_e( 'Unban this member', 'ap3-ion-theme' ); ?>"><?php _e( 'Remove Ban', 'ap3-ion-theme' ); ?></a>

							<?php else : ?>

								<a href="<?php bp_group_member_ban_link(); ?>" class="button confirm member-ban" title="<?php esc_attr_e( 'Kick and ban this member', 'ap3-ion-theme' ); ?>"><?php _e( 'Kick &amp; Ban', 'ap3-ion-theme' ); ?></a>
								<a href="<?php bp_group_member_promote_mod_link(); ?>" class="button confirm member-promote-to-mod" title="<?php esc_attr_e( 'Promote to Mod', 'ap3-ion-theme' ); ?>"><?php _e( 'Promote to Mod', 'ap3-ion-theme' ); ?></a>
								<a href="<?php bp_group_member_promote_admin_link(); ?>" class="button confirm member-promote-to-admin" title="<?php esc_attr_e( 'Promote to Admin', 'ap3-ion-theme' ); ?>"><?php _e( 'Promote to Admin', 'ap3-ion-theme' ); ?></a>

							<?php endif; ?>

								<a href="<?php bp_group_member_remove_link(); ?>" class="button confirm" title="<?php esc_attr_e( 'Remove this member', 'ap3-ion-theme' ); ?>"><?php _e( 'Remove from group', 'ap3-ion-theme' ); ?></a>
		...
		<?php else: ?>
			...
			<div id="message" class="info">
				<p class="padding"><?php _e( 'This group has no members.', 'ap3-ion-theme' ); ?></p>
			</div>
...
<?php if ( bp_is_group_admin_screen( 'delete-group' ) ) : ?>
	...
	<div class="padding">
		...
		<div id="message" class="info">
			<p><?php _e( 'WARNING: Deleting this group will completely remove ALL content associated with it. There is no way back, please be careful with this option.', 'ap3-ion-theme' ); ?></p>
		</div>

		<label><input type="checkbox" name="delete-group-understand" id="delete-group-understand" value="1" onclick="if(this.checked) { document.getElementById('delete-group-button').disabled = ''; } else { document.getElementById('delete-group-button').disabled = 'disabled'; }" /> <?php _e( 'I understand the consequences of deleting this group.', 'ap3-ion-theme' ); ?></label>

		<?php do_action( 'bp_after_group_delete_admin' ); ?>

		<div class="submit padding">
			<input type="submit" class="button button-primary" disabled="disabled" value="<?php esc_attr_e( 'Delete Group', 'ap3-ion-theme' ); ?>" id="delete-group-button" name="delete-group-button" />
		</div>
	</div>
