...
function interface_theloop() {
    ...
    elseif( is_singular( 'ministry' ) ) {
      interface_theloop_for_preach_the_word();
    }
    elseif( is_single() ) {
...
if ( ! function_exists( 'interface_theloop_for_preach_the_word' ) ) :
/**
 * Function to show the single post content.
 */
function interface_theloop_for_preach_the_word() {
  global $post;
...
<section id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
  <article>
    ...
    <header class="entry-header">
      ...
      <?php if(in_category( 167,168 ) || is_page( 6024 ) ) { 
          SDPopup(1); 
      } ?>
...
function streaming_video_body_class( $classes ) {
    if ( is_page_template( 'page-template-streaming-video.php' ) ) {
      $classes[] = 'faith-baptist-church-streaming-video-page';
    }
    return $classes;
}
