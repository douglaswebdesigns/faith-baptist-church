...
/**
 * Register "Prison Ministry Table" post type
 * @return [type] [description]
 */
function dh_ptp_register_pricing_table_post_type() {

	$labels = array(
	    'name' => __('Prison Ministry Tables', 'easy-pricing-tables'),
	    'singular_name' => __('Prison Ministry Table', 'easy-pricing-tables'),
	    'add_new' => __('Add New', 'easy-pricing-tables'),
	    'add_new_item' => __('Add New Prison Ministry Table', 'easy-pricing-tables'),
	    'edit_item' => __('Edit Prison Ministry Table', 'easy-pricing-tables'),
	    'new_item' => __('New Prison Ministry Table', 'easy-pricing-tables'),
	    'all_items' => __('All Prison Ministry Tables', 'easy-pricing-tables'),
	    'view_item' => __('View Prison Ministry Table', 'easy-pricing-tables'),
	    'search_items' => __('Search Prison Ministry Tables', 'easy-pricing-tables'),
	    'not_found' =>  __('No Prison Ministry Tables found', 'easy-pricing-tables'),
	    'not_found_in_trash' => __('No Prison Ministry Tables found in Trash', 'easy-pricing-tables'),
	    'parent_item_colon' => '',
	    'menu_name' => __('Prison Ministry Tables', 'easy-pricing-tables')
	  );
 ...
 * eg: Changes "Post published" to "Prison Ministry Table published"
...
function dh_ptp_updated_interaction_messages( $messages ) {
	...
	$messages['easy-pricing-table'] = array(
		0 => '', 
		1 => sprintf( __('Prison Ministry table saved. <a href="%s">View Prison Ministry table</a>.', 'easy-pricing-tables'), esc_url( get_permalink($post_ID) ) ),
		2 => __('Custom field updated.', 'easy-pricing-tables'),
		3 => __('Custom field deleted.', 'easy-pricing-tables'),
		4 => __('Prison Ministry table saved.', 'easy-pricing-tables'),
		5 => isset($_GET['revision']) ? sprintf( __('Prison Ministry table restored to revision from %s', 'easy-pricing-tables'), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
		6 => sprintf( __('Prison Ministry table saved. <a href="%s">View Prison Ministry table</a>', 'easy-pricing-tables'), esc_url( get_permalink($post_ID) ) ),
		7 => __('Prison Ministry table saved.', 'easy-pricing-tables'),
		//8 => sprintf( __('Prison Ministry table submitted. <a target="_blank" href="%s">Preview Prison Ministry table</a>'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
		8 => __('Prison Ministry table submitted.', 'easy-pricing-tables'),
		9 => sprintf( __('Prison Ministry table scheduled for: <strong>%1$s</strong>.', 'easy-pricing-tables'), date_i18n( __( 'M j, Y @ G:i' ), strtotime( $post->post_date ) ), esc_url( get_permalink($post_ID) ) ),
		10 => __('Prison Ministry table saved.', 'easy-pricing-tables'),
	);
	return $messages;
}
...
/**
 * changes the "Enter title here" to "Enter Prison Ministry table name here'" for pricing-table post type
 */
...
function dh_ptp_custom_rewrites($translation, $text, $domain)
{
	...
	switch ($post->post_type) {
		case 'easy-pricing-table': // enter your post type name here
			$translation_array = array(
				'Enter title here' => 'Enter Prison Ministry table month here'
			);
			break;
	}
...
/**
 * Customize Prison Ministry table overview tables ("Prison Ministry Tables" -> "All Prison Ministry Tables")
 * Add / modify columns at Prison Ministry table edit overview
 * @param  [type] $gallery_columns [description]
 * @return [type]                  [description]
 */
function dh_ptp_add_new_pricing_table_columns($gallery_columns) {
    $new_columns['cb'] = '<input type="checkbox" />';
    $new_columns['title'] = _x('Prison Ministry Table Name', 'column name', 'easy-pricing-tables');    
...
(Append the Prison Ministry table shortcode to the empty post.)
...
/**
 * Remove the publish metabox for Prison Ministry tables
 * @return [type] [description]
 */
...
function dh_ptp_save_tab_state( $post_id ) {
	...
	// Check if post type matches the Prison Ministry tables
...
/**
 * Print accordion related JS in Prison Ministry tables create/edit pages
 */
