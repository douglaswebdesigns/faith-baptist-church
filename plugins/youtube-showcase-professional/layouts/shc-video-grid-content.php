...
<?php switch (emd_mb_meta('emd_video_list_type')) {
	...
	break;
} ?> <?php echo (($video_grid_count > 1 and ($video_grid_count % 2 == 0 or $video_grid_count % 3 == 0)) ? '
    <div class="clearfix ' : ''); ?> <?php echo (($video_grid_count % 2 == 0 and $video_grid_count != 0) ? 'visible-sm-block' : ''); ?> <?php echo (($video_grid_count % 3 == 0 and $video_grid_count != 0) ? 'visible-md-block' : ''); ?> <?php echo (($video_grid_count % 4 == 0 and $video_grid_count != 0) ? 'visible-lg-block' : ''); ?> <?php echo (($video_grid_count > 1 and ($video_grid_count % 2 == 0 or $video_grid_count % 3 == 0)) ? '"></div>
    ' : ''); ?> 
