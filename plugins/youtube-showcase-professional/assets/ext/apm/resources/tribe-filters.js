jQuery(document).ready(function($) {
	...
	// Datetimepicker
	active.delegate('.tribe-datetimepicker', 'focusin', function(event) {
		format = 'yy-mm-dd';
		...
		if($(event.target).attr('rel-time'))
		{
			time_format = $(event.target).attr('rel-time');
		}
		$(this).datetimepicker({
			dateFormat: format,
	...
	//not exists on attributes, if not exists selected hide value but set to 1
	active.delegate('.tribe-filters-select','change',function(event){
		if($(event.target).val() == 'nexist'){
			$(".tribe-filters-input").val('1');
			$(".tribe-filters-input").hide();
		}
		else {
			$(".tribe-filters-input").val('');
			$(".tribe-filters-input").show();
		}
	});
});
