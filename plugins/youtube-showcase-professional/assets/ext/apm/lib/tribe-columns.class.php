...
class Tribe_Columns {
	...
	public function output_form() {
		...
		<ul id="tribe-cols-active"><?php
			$i = 1;
			foreach( $this->active as $v ) {
				if(isset($headers[$v]) && $v != 'comments')
				{
					echo '<li>';
					echo '<input type="hidden" name="'.$this->prefix.$i.'" value="'.$v.'" />';
					echo $headers[$v];
					echo '<b class="close">x</b>';
					echo '</li>';
					$i++;
				}
			}
...
<script> var Tribe_Columns = <?php echo json_encode( array(
		'prefix' => $this->prefix,
		'item' => '<li>%name%<b class="close">x</b></li>',
	...
	public function save_active() {
		...
		if ( isset($_POST['tribe-clear']) ) {
			//removed code not to clear the column order with filter clean
			//$this->reset_active();
			return;
		}
	...
	public function custom_columns( $column_id, $post_id ) {
		...
		if ( array_key_exists( $column_id, $this->columns ) ) {
			...
			if ( isset( $column['meta'] ) ) {
				...
				if(in_array($column['type'],Array('plupload_image','image','thickbox_image')))
                                {
                                        $image_list = emd_mb_meta($column['meta'],'type=image');
                                        if(!empty($image_list))
                                        {
						$value = "";
						foreach($image_list as $myimage)
                                ...
                                {
                                        $file_list = emd_mb_meta($column['meta'],'type=file');
                                        if(!empty($file_list))
                                        {
						$value = "";
				...
				elseif(in_array($column['type'],Array('user','user-adv')))
                                {
                                        $user_id = emd_mb_meta($column['meta']);
                                        if(!empty($user_id))
                                        {
                                ...
                                elseif($column['type']  == 'checkbox_list')
                                {
                                        $checkbox_list = emd_mb_meta($column['meta'],'type=checkbox_list');
                                        if(!empty($checkbox_list))
                                        {
                                                $value = implode(', ', $checkbox_list);
                                        }
                                }
				elseif($column['type']  == 'select' || $column['type']  == 'select_advanced')
                                {
                                        $select_list = get_post_meta( $post_id, $column['meta'], false );
			...
			else if ( isset( $column['custom_type'] ) && $column['custom_type'] == 'comment') {
					$value = '<a href="#" title="' . __('No Comments','emd-plugins') . '"><span style="color:black;" class="dashicons dashicons-welcome-comments"></span></a>';
					$lcomment = get_post_meta($post_id,'wpas_last_comment',true);
					if(!empty($lcomment)){
						$latest_comment = get_comments(array('number' => 1,'post_id'=>$post_id, 'orderby' => 'ASC','type'=>$column['comment_type']));
						if(!empty($latest_comment)){
							$value = '<a href="' . get_permalink() . '/#comments" title="' . $latest_comment[0]->comment_author . '"><span data-comment-role="' . $lcomment . '" class="dashicons dashicons-admin-comments"></span></a>';
						}
					}
			}
			else if ( isset( $column['custom_type'] ) && $column['custom'] == 'relationship' && $column['custom_type'] == 'rel' ) {
				$value = $this->relationship_column($post_id, $column);
			}
			...
			$first_col = $this->active[0];          
                        if($first_col == 'date')
                        {
                                $first_col = $this->active[1];
                        }

			// if name isn't set, and first in $this->columns, let's link it up
			//if title is not in columns , change first field to editable
			//if ( ! in_array( 'title', $this->active ) && reset($this->active) == $column_id ) {
			if ( ! in_array( 'title', $this->active ) && $column_id == $first_col) {
	...
	private function get_column_headers($omit_checkbox = true) {
		...
		{
			$this->screen->post_type = $this->post_type;
		}
	...
	protected function taxonomy_column($post_id, $taxonomy) {
		...
		foreach ( $terms as $term ) {
			$url = "/" . $taxonomy . "/" . $term->slug;
			$ret[] = "<span class='taxlabel' style='font-size:100%;' data-tax-" . str_replace('_','-',$taxonomy) . "='" . $term->slug . "'>" . sprintf('<a href="%s" target="_blank">%s</a>', $url, $term->name) . "</span>"; 
		}
		return implode(', ', $ret);
	}
	

	protected function relationship_column($post_id,$column) {
		$connected = p2p_type($column['rel_type'])->get_connected($post_id,array('posts_per_page'=>-1));	
		if(empty($connected->posts)) return '&ndash;';
		$ent_list = get_option(str_replace("-","_",$column['app_name']) . '_ent_list');
		if(!empty($ent_list[$column['conn_ptype']])){
			$meta_keys = $ent_list[$column['conn_ptype']]['unique_keys'];
		}
		if(!isset($meta_keys) || empty($meta_keys) ) return '&ndash;';
		$ret = array();
		foreach($connected->posts as $myrelpost) {
			$rel_value = '';
			if(count($meta_keys) == 1 && isset($ent_list[$column['conn_ptype']]['user_key']) && $ent_list[$column['conn_ptype']]['user_key'] == $meta_keys[0])
			{
				$tpart = emd_mb_meta($ent_list[$column['conn_ptype']]['user_key'], Array() , $myrelpost->ID);
				$user_info = get_userdata($tpart);
				$rel_value = $user_info->display_name;
			}
			else {
				foreach($meta_keys as $mymeta_key) {
					if(preg_match('/blt_title/',$mymeta_key)){
						$rel_value .= get_the_title($myrelpost->ID);
					}
					else {
						$rel_value .= get_post_meta($myrelpost->ID,$mymeta_key,true) . " ";
					}
				}
			...
			$ret[] = sprintf('<a href="%s" title="%s" target="_blank">%s</a>', $url,$rel_title,$rel_value);
		}
		return implode(', ', $ret);
	}
	...
	protected function is_our_post_type() {
		...
		if ( empty($screen) ) {
			global $pagenow;
			if('admin-ajax.php' == $pagenow && $_POST['action'] == 'inline-save'  && 'list' == $_POST['post_view'])
			{
