...
class Tribe_Filters {
	...
	private $app_name;
	private $textdomain = 'emd-plugins';
	...
	private $binary_query_options = array(
                'exist' => 'Exists',
                'nexist' => 'Not Exists',
        );
	private $comment_query_options = array(
                'none' => 'No Comments',
		'other' => 'Last Comment By Other',
        );
	...
	private $query_text_search_options = array(
		'like' => 'Search',
		'is' => 'Is',
		'not' => 'Is Not',
		'nexist' => 'Not Exists',
	...
	public function __construct( $post_type, $app_name, $filters = array() ) {
...
#		add_action( 'shutdown', array($this, 'debug') );
	...
	public function input_date_class($class, $filter) {
		if ( self::is_date($filter)  == 'date') {
			return 'date tribe-datepicker';
		}
		elseif ( self::is_date($filter)  == 'datetime') {
			return 'date tribe-datetimepicker';
		}
		return $class;
	}
	...
	public function parse_query($wp_query) {
		...
		foreach ($this->active as $k => $v ) {
			...
			if ( isset($filter['taxonomy']) ) {
				...
				if($filter['custom'] == 'relationship'){
					$rel_query[$k] = $v['value'];
				}
				else if($filter['custom'] == 'comment'){
					$comment_query[$k] = $v['value'];
				}
			}
			else if ( isset($filter['author'] ) ) {
				$author_query = $v['value'];
			}
		}

		if(!empty($comment_query)){
			switch($comment_query['latest_comment']){
				case 'none':
					$meta_query[] = array('key'=>'wpas_last_comment','compare'=>'NOT EXISTS');
					break;
				default:
					$meta_query[] = array('key'=>'wpas_last_comment','value' => $comment_query['latest_comment'], 'compare'=>'IN');
					break;
			}
		}

		if(!empty($tax_query) || !empty($meta_query) || !empty($rel_query) || !empty($author_query)){
                        $wp_query->set('apm_filter',1);
                }
		...
		if(count($rel_query) > 0){
			$emd_query = new Emd_Query($this->filtered_post_type,str_replace('-','_',$this->app_name));	
			foreach($rel_query as $krel => $vrel){
				$field_list[1] = ltrim($krel,'rel-');
				$field_list[3] = $vrel;
				$emd_query->get_rel_query($field_list);
			}
			if(isset($emd_query->args['post__in'])){
				if(empty($emd_query->args['post__in'])){
					$emd_query->args['post__in'] = Array(0);
				}
				$wp_query->set('post__in',$emd_query->args['post__in']);
			}
		}

		if(is_array($author_query)){
			$wp_query->set('author',implode(",",$author_query));
		}
		else{
			$wp_query->set('author',$author_query);
		}
	...
	public function save_active() {
		...
		foreach ( $this->filters as $key => $filter ) {
			...
			if ( isset($filter['meta']) ) {
				if(in_array($filter['type'],array('user','user-adv')))
                                {
			...
			// Custom types
				...
				$maybe_active = $this->maybe_active_taxonomy($key, $filter);
				//$maybe_active = apply_filters( 'tribe_maybe_active_' . $filter['custom_type'], false, $key, $filter );
			}
			else if ( isset($filter['author']) ) {
				$maybe_active = $this->maybe_active_taxonomy($key, $filter);
			}
	...
	public function enqueue() {
		...
		if ( $current_screen->id == 'edit-' . $this->filtered_post_type ) {
			wp_enqueue_script('tribe-filters', $resources_url . 'tribe-filters.js', array('jquery-ui-sortable'), null, true );
		}
	}
	...
	protected function saved_filters_dropdown() {
		...
		$url = esc_url(add_query_arg( array('post_type' => $this->filtered_post_type, 'saved_filter' => 'balderdash' ), admin_url('edit.php') ));
	...
	protected function table_row($key, $value) {
		...
		$before = '<tr><th scope="row"><b class="close">x</b>'.$filter['name'].'</th><td>';
		$after = '</td></tr>';
		if ( isset( $filter['taxonomy'] ) ) {
			...
			if(in_array($filter['type'],Array('user','user-adv'))) 
			{
				$ret = $this->author_row($key, $value, $filter);
			}
			else
			{
		...
		else if ( isset( $filter['custom_type'])  && $filter['custom_type'] == 'comment') {
			$value = array_merge( array('value' => 0), (array) $value );
                	$published_cap = get_post_type_object($this->filtered_post_type)->cap->edit_published_posts;
			$roles = get_editable_roles();
			foreach($roles as $keyrole => $myrole){
				if(array_key_exists($published_cap,$myrole['capabilities']) 
				&& $myrole['capabilities'][$published_cap] == true){
					$this->comment_query_options[$keyrole] = 'Last Comment By ' . $myrole['name'];
				}
			}	
			$ret = $this->select_field($this->prefix . $key, $this->comment_query_options, $value['value'], true);
		}
		else if ( isset( $filter['custom_type'])  && $filter['custom_type'] == 'rel') {
			//$opts = apply_filters( 'tribe_custom_row_' . $filter['custom_type'], '', $key, $value, $filter );
			$opts = $this->tribe_custom_row($key, $value, $filter);
			$value = array_merge( array('value' => 0), (array) $value );
			$ret = $this->select_field($this->prefix . $key, $opts, $value['value'], true);	
		}
		else if ( isset( $filter['author']) ) {
			$ret = $this->author_row($key, $value, $filter);
		}
	...
	protected function author_row($key, $value, $filter) {
		if(!empty($filter['roles']))
                {
                        $users = get_users(array('role'=>$filter['roles'][0]));
                        if(count($filter['roles']) > 1)
                        {
                                for($i=1;$i<count($filter['roles']);$i++)
                                {
                                        $users = array_merge($users,get_users(array('role'=>$filter['roles'][$i])));
                                }
                        }
                        asort($users);
                }
	...
	protected function meta_row($key, $value, $filter) {
		...
		if ( isset($filter['options']) && ! empty($filter['options']) ) {
			if($filter['cast'] == 'BINARY'){
				$query_options = apply_filters( 'tribe_query_options', $this->binary_query_options, $key, $filter );
				$ret .= $this->select_field( $is_key, $query_options, $value['query_option'] );
				$ret .= "<input type='hidden' name='{$val_key}'  value='1'>";
			}
			else {
				$query_options = apply_filters( 'tribe_query_options', $this->query_options, $key, $filter );
				$ret .= $this->select_field( $is_key, $query_options, $value['query_option'],false, 'tribe-filters-select');
				$ret .= $this->select_field( $val_key, $filter['options'], $value['value'], true, 'tribe-filters-input');
			}
		}
		// No explicit options. We're showing a search field
		else {
			if($filter['cast'] == 'CHAR'){
				$query_options = apply_filters( 'tribe_query_options', $this->query_text_search_options, $key, $filter );
			}
			else {
			...
			$ret .= $this->select_field( $is_key, $query_options, $value['query_option'],false,'tribe-filters-select' );
			$ret .= "<input type='text' name='{$val_key}' value='{$value['value']}'";
			if($input_class == 'date tribe-datepicker')
			{
				if(isset($filter['date_format']))
				{
					$ret .= " rel=" . $filter['date_format'];
				}
			}
			elseif($input_class == 'date tribe-datetimepicker')
			{
				if(isset($filter['date_format']))
				{
					$ret .= " rel=" . $filter['date_format'];
				}
				if(isset($filter['time_format']))
				{
					$ret .= " rel-time=" . $filter['time_format'];
				}
			}
			$ret .= " class='tribe-filters-input {$input_class}' ";
			if($value['query_option'] == 'nexist'){
	...
	public function select_field($name, $options = null, $active = '', $allow_multi = false, $myclass ='' ) {
		...
		$sel = '';
		if ( is_array($options) ) {
			$sel .= '<select id="'. $name .'" name="'. $name .'" class="' . $myclass . " " . $class . '"' . $multi . '>';
	...
	protected function meta_query($key, $val) {
		$filter = $this->filters[$key];
		if(isset($filter['time_format'])) {
			$format['type'] = $filter['type'];
			$format['dformat']['timeFormat'] = $filter['time_format'];
		}
		if(isset($filter['date_format'])) {
			$format['type'] = $filter['type'];
			$format['dformat']['dateFormat'] = $filter['date_format'];
			$value = emd_translate_date_format($format,$val['value']);
		}
		else {
			$value = $val['value'];
		}
	...
	protected function tribe_custom_row($key,$value,$filter)
	{
		$opts = get_posts(array('post_type' => $filter['conn_ptype'],'numberposts' => -1,'apm_custom'=>1));
		$meta_keys = Array();
		$myposts = Array();
		
		$ent_list = get_option(str_replace("-","_",$filter['app_name']) . '_ent_list');
		if(!empty($ent_list[$filter['conn_ptype']]))
		{
			$meta_keys = $ent_list[$filter['conn_ptype']]['unique_keys'];
		}
		if(!empty($opts) && !empty($meta_keys))
		{
			foreach($opts as $myopt)
			{
				$rel_value = "";
				if(count($meta_keys) == 1 && isset($ent_list[$filter['conn_ptype']]['user_key']) && $ent_list[$filter['conn_ptype']]['user_key'] == $meta_keys[0])
				{
					$tpart = emd_mb_meta($ent_list[$filter['conn_ptype']]['user_key'], Array() , $myopt->ID);
					$user_info = get_userdata($tpart);
					$rel_value = $user_info->display_name;
				}
                                else {
					foreach($meta_keys as $mymeta_key)
					{
						$rel_value .= get_post_meta($myopt->ID,$mymeta_key,true) . " - ";
					}
					$rel_value = rtrim($rel_value," - ");
				}
				$myposts[$myopt->ID] = $rel_value;
			}
		}
		return $myposts;
	}
